...
public class Partida {
    ...
    public Map<Pais,Set<Integer>> atacar(Pais atacante, Pais defensor) {
        ...
        valoresDadosAtacante = valoresDadosAtacante.stream().map(num -> rand.nextInt(6) + 1).collect(Collectors.toSet());
        valoresDadosDefensor = valoresDadosDefensor.stream().map(num -> rand.nextInt(6) + 1).collect(Collectors.toSet());
        ...
        while (!valoresDadosAtacante.isEmpty() && !valoresDadosDefensor.isEmpty() && !(defensor.getNumEjercitos()==0)) {
