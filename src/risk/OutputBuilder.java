...
public class OutputBuilder {
    ...
    for (Field f : obj.getClass().getFields()) { // Recorremos todos los campos del objeto
      try {
        stringBuilder
          ...
          .append(f.getName())
          .append(": \"")
          .append(f.get(obj).toString())
          .append("\"")
          .append(NEW_LINE);
      } catch (IllegalArgumentException | IllegalAccessException e) { // En caso de que haya un error...
        Logger
          .getLogger(OutputBuilder.class.getName())
          .log(
            Level.WARNING,
            "Excepción al convertir el campo {0} a un String",
            f.getName()
          );
      }
    }
  ...
  public static String buildFromObjectGetters(Object obj, int depthLevel) {
    ...
    for (Method m : obj.getClass().getMethods()) { // Por cada método del objeto...
      if (
        !m.getName().toLowerCase().contains("class") &&
        m.getName().toLowerCase().startsWith("get") &&
        (m.getParameterCount() == 0)
      ) { // El método tiene que empezar por get. Aparte, el método no
   ...
   * @return
   */
  private static String buildFromObjectGetters(
    Object obj,
    int depthLevel,
    int cantidadDeSangrado
  ) {
      ...
      if (
        ...
        try {
          ...
          try { // Tratamos de construir una cadena con el nombre del getter en formato humano.
            ...
            builderNombreObjeto.append(
              Character.toString(nombreObjeto.charAt(0)).toLowerCase()
            );
          ...
          stringBuilder
            .append(new String(new char[cantidadDeSangrado]).replace('\0', ' '))
            .append(nombreObjeto)
            .append(": "); // Añadimos el nombre del objeto al
          // JSON
          ...
          if (Iterable.class.isAssignableFrom(m.getReturnType())) {
            ...
            int cantidadDeSangradoLocal = stringBuilder
              .toString()
              .split(NEW_LINE)[stringBuilder.toString().split(NEW_LINE).length -
              1].length(); // Vamos
            // a
            // mirar
            // dónde
            ...
            Iterator<Object> iterator =
              ((Iterable<Object>) m.invoke(obj)).iterator();
            while (iterator.hasNext()) {
              stringBuilder
                .append("\"")
                .append(
                  OutputBuilder.buildFromObjectGetters(
                    iterator.next(),
                    cantidadDeSangradoLocal
                  )
                )
                .append("\"");
              if (iterator.hasNext()) { // Si este no es el último elemento, preparamos la siguiente línea
                ...
                // línea
                stringBuilder.append(
                  new String(new char[cantidadDeSangradoLocal])
                  .replace('\0', ' ')
                ); // Añadimos
                // una
                // nueva
            ...
            stringBuilder.append(
              new String(new char[cantidadDeSangradoLocal - 2])
              .replace('\0', ' ')
            ); // 2
            // caracteres
            // de
          ...
          Logger
            ...
            .log(
              Level.WARNING,
              "Excepción al obtener el valor de {0}",
              m.getName()
            );
        }
      }
  ...
  private OutputBuilder addVariableUsingGetter(Object obj, Method m) {
    ...
    if (Iterable.class.isAssignableFrom(m.getReturnType())) {
      ...
      if (DEPTH_LEVEL > 0) { // Si DEPTH_LEVEL es > 0, seguimos desencapsulando
        stringBuilder.append(
          anadirSangrado(
            OutputBuilder.buildFromObjectGetters(valorObjeto, DEPTH_LEVEL - 1),
            sangrado
          )
        ); // Sangrado
        // +
  ...
  private String anadirSangrado(String stringOriginal, int cantidadDeSangrado) {
    return stringOriginal.replace(
      NEW_LINE,
      NEW_LINE + new String(new char[cantidadDeSangrado]).replace('\0', ' ')
    );
  }
  ...
  private String getListFromIterable(Iterable<Object> iterable) {
    ...
    while (iterator.hasNext()) {
      ...
      if (DEPTH_LEVEL > 0) { // Si DEPTH_LEVEL es > 0, seguimos desencapsulando
        stringBuilder.append(
          OutputBuilder.buildFromObjectGetters(currentObject, DEPTH_LEVEL - 1)
        );
      } else { // Si DEPTH_LEVEL NO es > 0, así que usamos toString()
  ...
  public String build() {
    ...
    while (iterator.hasNext()) {
      stringBuilder.append(
        new String(new char[nivelSangrado]).replace('\0', ' ')
      );
