...
public class Coordenadas implements Comparable {
    ...
    public int compareTo(Object o) {
        ...
        if (this.getY() < coords.getY()) // La coordenada de este objeto está más arriba, así que devolvemos que es
                                         // menor
            return -1;

        if (this.getX() > coords.getX()) // La coordenada de este objeto está en el mismo nivel, pero más a la derecha,
                                         // así que devolvemos que es mayor
            return 1;

        if (this.getX() < coords.getX()) // La coordenada de este objeto está en el mismo nivel, pero más a la
                                         // izquierda, así que devolvemos que es menor
            return -1;
